...
namespace DG.Core.Behaviors.Common
{
    internal sealed class DGEquipmentBehavior : IDGBehaviour
    {
        ...
        {
            entity.ComponentContainer.TryGetComponent(out inventoryComponent);
            entity.ComponentContainer.TryGetComponent(out equipmentComponent);
            ...
            DGWeapon[] weapons = inventoryComponent.Items.Where(x => x.GetType().IsSubclassOf(typeof(DGWeapon))).Cast<DGWeapon>().ToArray();
